...
public class SparseTriangularMatrix{
	...
	/**
	 * Get all supports from the matrix, sorted in descending order.
	 * Each support value corresponds to a candidate 2-subgraph (vertex-label pair).
	 * @return a list of support values sorted in descending order
	 */
	public List<Integer> getAllSupportsSorted() {
		List<Integer> supports = new ArrayList<>();
		for (Map<Integer, Integer> innerMap : matrix.values()) {
			supports.addAll(innerMap.values());
		}
		supports.sort(Collections.reverseOrder());
		return supports;
	}

	/**
	 * Get all entries (labelI, labelJ, support) from the matrix.
	 * Each entry represents a candidate 2-subgraph.
	 * @return a list of int arrays, each containing [labelI, labelJ, support]
	 */
	public List<int[]> getAllEntries() {
		List<int[]> entries = new ArrayList<>();
		for (Map.Entry<Integer, Map<Integer, Integer>> outerEntry : matrix.entrySet()) {
			int i = outerEntry.getKey();
			for (Map.Entry<Integer, Integer> innerEntry : outerEntry.getValue().entrySet()) {
				int j = innerEntry.getKey();
				int support = innerEntry.getValue();
				entries.add(new int[]{i, j, support});
			}
		}
		return entries;
	}
	...
	/**
	 * This method is for a single graph database to updata vertex pair count
	 * because vertex pairs are double counted
	 */
	public void updateSup(){
		for (Entry<Integer, Map<Integer, Integer>> entry : matrix.entrySet()) {
			Iterator<Entry<Integer,Integer>> iter = entry.getValue().entrySet().iterator();
			while (iter.hasNext()){
				Entry<Integer,Integer> entry2 = iter.next();
				int newSup = entry2.getValue()/2;
				setSupport(entry.getKey(), entry2.getKey(), newSup);
			}
		}
	}
}
