...
public class AlgoDP {
    ...
    /** the maximum edge number of frequent patterns */
    ...
    /** use the hypertree method */
	boolean getHypertree = false;

    /** the number of cadidate patterns */
    int candidateNum = 0;
...
//    private double epsilon = 1;
    ...
    private static boolean isNaive = false;
	/**
	 ...
	 */
	public void runAlgorithmDP(String inPath, String outPath, int minSupport, String chooseSup, boolean outputSingleVertices,
                               boolean outputDotFile, int maxNumberOfEdges, boolean isNaive, boolean getHypertree, double epsilon)
...
//        maxNumberOfEdges = (int) ((double) maxNumberOfEdges + Distribution.laplace(0.1, 1));
		...
		// Find the maximum pattern size among all frequent subgraphs
		patternMaxSize = 0;
		for (FrequentSubgraph subgraph : frequentSubgraphs) {
			int size = subgraph.dfsCode.size();
			if (size > patternMaxSize) {
				patternMaxSize = size;
			}
		}
...
//        System.out.println("database edge number: " + graphDB.getEdgeCount());
	...
	/**
	 * Binary Estimation: estimate the number of frequent 2-subgraphs (n_2)
	 * using the sparse triangular matrix and binary search with Laplace noise.
	 *
	 * @param matrix  the sparse triangular matrix with support counts
	 * @param eps_a   privacy budget for binary estimation
	 * @param tau     support threshold (minSup)
	 * @return the estimated number of frequent 2-subgraphs n_2
	 */
	private int binaryEstimation(SparseTriangularMatrix matrix, double eps_a, int tau) {
		// Get all supports sorted in descending order
		List<Integer> supports = matrix.getAllSupportsSorted();
		int totalCandidates = supports.size();
		...
		int low = 0;
		int high = totalCandidates - 1;

		// Sensitivity: ceil(log2(|C_2|))
		double sensitivity = Math.ceil(Math.log(totalCandidates) / Math.log(2));

		while (low <= high) {
			int mid = (low + high) / 2;

			// s_m = support of the m-th largest candidate (0-indexed)
			int s_m = supports.get(mid);

			// Add Laplace noise: ns_m = s_m + Lap(ceil(log2|C_2|) / eps_a)
			double ns_m = s_m + Distribution.laplace(eps_a, (int) sensitivity);

			if (ns_m == tau) {
				return totalCandidates - mid;
			} else if (ns_m > tau) {
				high = mid - 1;
			} else {
				low = mid + 1;
		...
		// Return |C_2| - 1 - high
		return totalCandidates - 1 - high;
	}

	/**
	 * Conditional Exponential Mechanism: select n_2 frequent 2-subgraphs
	 * using a combination of Laplace noise and exponential mechanism.
	 *
	 * @param allEntries all candidate 2-subgraph entries [labelI, labelJ, support]
	 * @param eps_b      privacy budget for conditional exponential mechanism
	 * @param tau        support threshold (minSup)
	 * @param n_2        the number of frequent 2-subgraphs to select
	 * @return a set of selected frequent 2-subgraph entries [labelI, labelJ, support]
	 */
	private List<int[]> conditionalExponentialMechanism(List<int[]> allEntries, double eps_b, int tau, int n_2) {
		// Split eps_b into eps_b1 and eps_b2 (beta = 0.5 for equal split)
		double beta = 0.5;
		double eps_b1 = beta * eps_b;
		double eps_b2 = (1 - beta) * eps_b;

		List<int[]> C_2 = new ArrayList<>(allEntries);
		List<int[]> F_2 = new ArrayList<>();

		if (n_2 <= 0 || C_2.isEmpty()) {
			return F_2;
		}

		for (int j = 0; j < n_2; j++) {
			List<int[]> C_prime = new ArrayList<>();

			// Step 1: Add Laplace noise to each candidate's support
			for (int[] entry : C_2) {
				int s_e = entry[2]; // support
				double ns_e = s_e + Distribution.laplace(eps_b1, 2 * n_2);
			...
			// Step 2: Select one edge from C_prime using exponential mechanism
			if (!C_prime.isEmpty()) {
				// Compute weights: exp(eps_b2 * s_e / (2 * n_2))
				double[] weights = new double[C_prime.size()];
				double totalWeight = 0;
				for (int k = 0; k < C_prime.size(); k++) {
					int s_e = C_prime.get(k)[2];
					weights[k] = Math.exp(eps_b2 * s_e / (2.0 * n_2));
					totalWeight += weights[k];
				}

				// Sample according to weights
				double rand = Distribution.rd.nextDouble() * totalWeight;
				double cumulative = 0;
				int selectedIdx = 0;
				for (int k = 0; k < C_prime.size(); k++) {
				...
				int[] selected = C_prime.get(selectedIdx);

				// Remove from C_2 and add to F_2
				C_2.remove(selected);
				F_2.add(selected);
			}
		}
...
//			Vertex[] vertices = graphDB.getAllVertices();
                ...
                for (int i = 0; i < vertices.length; i++) {
                    ...
                    while (iter.hasNext()) {
                        ...
                        // add noise to 'count' and 'mapEdgeLabelToSupport.get(edge.getEdgeLabel())'
                        // example for infrequent pair and edge label { pair:[a-b], label: [A] }
                        // eliminating pairs and edge labels guarantees the removal of infrequent 2-subgraphs

                        double ncount = count + Distribution.laplace(e1/2,graphDB.getEdgeCount());
                        double ncountLabel = mapEdgeLabelToSupport.get(edge.getEdgeLabel()) + Distribution.laplace(e1/2,graphDB.getEdgeCount());
                        if (ELIMINATE_INFREQUENT_VERTEX_PAIRS && ncount < minSup) {
                            iter.remove();

                            infrequentVertexPairsRemoved++;
                        } else if (ELIMINATE_INFREQUENT_EDGE_LABELS && ncountLabel < minSup) {
                            iter.remove();
                            edgeRemovedByLabel++;
                        }
                    }
            ...
            // Smart DP algorithm: EliminateEdges-Smart
            if (ELIMINATE_INFREQUENT_VERTEX_PAIRS || ELIMINATE_INFREQUENT_EDGE_LABELS) {
                // epsilon_a and epsilon_b: split e1 equally
                double alpha = 0.5;
                double eps_a = alpha * e1;
                double eps_b = (1 - alpha) * e1;

                // Step 1: Binary Estimation to get n_2 (estimated number of frequent 2-subgraphs)
                int n_2 = binaryEstimation(matrix, eps_a, minSup);

                // Step 2: Get all candidate 2-subgraphs from the matrix
                List<int[]> allEntries = matrix.getAllEntries();

                // Step 3: Conditional Exponential Mechanism to select frequent 2-subgraphs
                List<int[]> frequentEntries = conditionalExponentialMechanism(allEntries, eps_b, minSup, n_2);

                // Build a set of frequent (labelI, labelJ) pairs for fast lookup
                Set<String> frequentPairSet = new HashSet<>();
                for (int[] entry : frequentEntries) {
                    int i = entry[0];
                    int j = entry[1];
                    frequentPairSet.add(i + "," + j);
                }

                // Step 4: Remove edges whose vertex-label pair is NOT in the frequent set
                for (int i = 0; i < vertices.length; i++) {
                    Vertex v1 = vertices[i];
                    int labelV1 = v1.getLabel();
                    ...
                    while (iter.hasNext()) {
                        ...
                        String pairKey = (labelV1 < labelV2) ? labelV1 + "," + labelV2 : labelV2 + "," + labelV1;

                        if (ELIMINATE_INFREQUENT_VERTEX_PAIRS && !frequentPairSet.contains(pairKey)) {
                            iter.remove();
                            infrequentVertexPairsRemoved++;
                        } else if (ELIMINATE_INFREQUENT_EDGE_LABELS) {
                            // For edge labels, still use Laplace noise
                            double ncountLabel = mapEdgeLabelToSupport.get(edge.getEdgeLabel()) + Distribution.laplace(e1/2, graphDB.getEdgeCount());
                            if (ncountLabel < minSup) {
                                iter.remove();
                                edgeRemovedByLabel++;
                            }
                        }
...
//			}
	...
	private void gSpanDFS(DFSCode c, Set<HyperEdge> hyperEdges, Graph graphDB)
        ...
        candidateNum = candidateNum + extensions.size();
		...
		if (extensions != null) {
            for (Entry<ExtendedEdge, Set<HyperEdge>> entry : extensions.entrySet()) {
                ...
                if (isNaive){
                    double eps_dfs = e2 / maxNumberOfEdges;
                    double nsup = sup + Distribution.laplace(eps_dfs / extensions.size(), 1);
                    ...
                    if (nsup >= minSup) {
                        ...
                        if (isCanonical(newC)) {
                            // Save the graph
                            FrequentSubgraph subgraph;
                            subgraph = new FrequentSubgraph(newC, hyperEdgeSet, (int) nsup);
                    ...
                    //1. Local uniqueness. Since each HyperEdge (embedding) is assigned to at most one sibling extension at each DFS node, the extension-specific candidate sets form a disjoint partition of the underlying embedding space.
                    //2. Global uniqueness. Due to the uniqueness-preserving embedding assignment induced by canonical DFS code ordering, each HyperEdge participates in at most one extension event across the entire DFS exploration.
                    //Therefore, the mechanism induces a global partition of the data, enabling pure parallel composition of Laplace mechanisms across all DFS nodes.
//                    double nsup_parallel = sup + Distribution.laplace(eps_dfs, 1);
                    // if the support is enough
                    if (sup >= minSup) {
                        ...
                        if (isCanonical(newC)) {
                            // Save the graph
                            FrequentSubgraph subgraph;
                            subgraph = new FrequentSubgraph(newC, hyperEdgeSet, sup);
...
//	}
	...
	public static int MVC(Set<HyperEdge> hyperEdgeSet) {
		...
		while (!uncoveredEdges.isEmpty()) {
			...
			int maxCover = -1;
			int vertexToAdd = -1;
			for (Entry<Integer, Integer> entry : vertexFrequency.entrySet()) {
...
//	}
    ...
    public void calculateNoisySup(){
        // Step 1: Count how many times each edge (identified by vLabel1, vLabel2, edgeLabel)
        // appears across all frequent subgraphs.
        // Key: "vLabel1,vLabel2,edgeLabel" (normalized so vLabel1 <= vLabel2)
        Map<String, Integer> edgeGlobalCount = new HashMap<>();

        for (FrequentSubgraph subgraph : frequentSubgraphs) {
            Set<String> seenEdgesInSubgraph = new HashSet<>();
            for (ExtendedEdge ee : subgraph.dfsCode.getEeL()) {
                int l1 = ee.getvLabel1();
                int l2 = ee.getvLabel2();
                // Normalize so that (l1, l2) is ordered consistently
                String key;
                if (l1 <= l2) {
                    key = l1 + "," + l2 + "," + ee.getEdgeLabel();
                } else {
                    key = l2 + "," + l1 + "," + ee.getEdgeLabel();
                }
                // Count each distinct edge type only once per subgraph
                if (seenEdgesInSubgraph.add(key)) {
                    edgeGlobalCount.put(key, edgeGlobalCount.getOrDefault(key, 0) + 1);
                }
            }
        }

        // Step 2: For each frequent subgraph, find its maximum edge count m,
        // allocate privacy budget e2/m, and add Laplace noise to its support.
        for (FrequentSubgraph subgraph : frequentSubgraphs) {
            int m = 0;
            Set<String> seenEdgesInSubgraph = new HashSet<>();
            for (ExtendedEdge ee : subgraph.dfsCode.getEeL()) {
                ...
                // Count each distinct edge type only once per subgraph
                if (seenEdgesInSubgraph.add(key)) {
                    int count = edgeGlobalCount.getOrDefault(key, 0);
                    if (count > m) {
                        m = count;
                    }
                }
            ...
            // Allocate privacy budget e2/m to this subgraph
            double eps_sub = (m > 0) ? e2 / m : e2;
            double noisySup = subgraph.support + Distribution.laplace(eps_sub, 1);
            subgraph.support = (int) Math.round(noisySup);
        }
    }
	...
	public void printStats() {
        ...
        System.out.println(" Maximum size (edge number) of frequent subagraphs: " + patternMaxSize);
